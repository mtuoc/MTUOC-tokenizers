...
class Tokenizer():
    def __init__(self):
        ...
        self.abr=["abe.","abu.","adib.","admin.","al.","and.","api.","aptu.","ar.","auz.","az.","aza.","b.","bas.","bl.","bul.","bulk.-zk.","dept.","eka.","enp.","enpr.","ent.","erd.","erp.","esk.","eskra.","etab.","etorb.","ezk.","ig.","ira.","jn.","jn.-and.ak","k.","l.","lr.","mai.","mar.","og.","or.","ots.","p.","pl.","pol.","prob.","sl.","tef.","tel.","urr.","urt.","uzt.","zenb.","z.g.","zk."]
        abr_aux=[]
        for a in self.abr:
            am1=a.capitalize()
            am2=a.upper()
    ...
    def main_tokenizer(self,segment):
        ...
        #retokenitzem tokens que acaben en puntuació
        
        toretoken=cadena.split()
        novacadena=[]
        for rt in toretoken:
            rtmod=rt
            for punt in self.punt:
                if rt.endswith(punt) and not rt in self.abr:
                    pmod=" ￭"+punt
                    rtmod=rt[0:-1]+pmod
            novacadena.append(rtmod)
        cadena=" ".join(novacadena)
        
        #retokenitzem hyphens:
        ...
        for rt in toretoken:
            ...
            for hyp in hyp2:
                tosplit=True
                if rtmod.find(hyp)>-1:
                    if regex.search(r'\p{L}', rtmod, regex.UNICODE): tosplit=False
                if tosplit:
                    hmod=" ￭"+hyp+" ￭"
                    rtmod=rtmod.replace(hyp,hmod)
...
def print_help():
    print("MTUOC_tokenizer_eus.py A tokenizer for Basque, usage:")
    print("Simple tokenization:")
    print('    cat "sentence to tokenize." | python3 MTUOC_tokenizer_eus.py tokenize')
    print('    python3 MTUOC_tokenizer_eus.py tokenize < file_to_tokenize > tokenized_file')
    print()
    print("Simple detokenization:")
    print('    cat "sentence to tokenize." | python3 MTUOC_tokenizer_eus.py detokenize')
    print('    python3 MTUOC_tokenizer_eus.py detokenize < file_to_detokenize > detokenized_file')
    print()
